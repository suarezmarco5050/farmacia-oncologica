...
st.set_page_config(page_title="Asistente Cardio-Oncológico", layout="wide")
st.title("🧠 Asistente Cardio-Oncológico")
st.markdown("Este asistente analiza el informe clínico de un paciente oncológico y genera recomendaciones personalizadas según el riesgo cardiovascular y el tipo de tratamiento recibido.")

# Cargar archivo PDF del paciente
uploaded_file = st.file_uploader("📄 Sube el informe clínico del paciente (PDF)", type=["pdf"])
...
# Función para evaluar riesgo cardiovascular
def evaluar_riesgo(texto):
    ...
    criterios = {
        ...
        "QT": "QT largo",
        "FEVI": "FEVI reducida",
        "RT torácica": "Radioterapia torácica",
        "edad": "Edad avanzada",
        "renal": "Enfermedad renal crónica",
        "cardiopatía": "Cardiopatía previa"
    }
...
# Función para detectar tipo de fármaco
...
def generar_recomendaciones(riesgo, farmacos):
    ...
    if riesgo in ["Alto", "Muy Alto"]:
        ...
        recomendaciones.append("- Evaluar cardioprotección: IECAs, betabloqueantes, ARNI, iSGLT2.")
    ...
    for grupo, f in farmacos:
        if grupo == "antraciclina":
            ...
            recomendaciones.append(f"- {f}: controlar PA, riesgo de QT prolongado y disfunción ventricular.")
        elif grupo == "PARP":
...
if uploaded_file:
    ...
    st.subheader("📋 Evaluación del Informe")
    st.text_area("Texto extraído del informe", value=texto, height=300)
    ...
    st.subheader("🩺 Estratificación de Riesgo Cardiovascular")
    st.markdown(f"**Nivel de riesgo:** {riesgo}")
    st.markdown(f"**Factores detectados:** {', '.join(factores) if factores else 'Ninguno'}")

    st.subheader("💊 Fármacos Detectados")
    if farmacos_detectados:
        for grupo, f in farmacos_detectados:
            st.markdown(f"- {f} ({grupo})")
    else:
        st.markdown("No se detectaron fármacos cardiotóxicos en el informe.")

    st.subheader("📑 Recomendaciones Personalizadas")
    for r in recomendaciones:
        st.markdown(f"- {r}")

    st.subheader("📚 Herramientas de Apoyo")
    st.markdown("- [Drugs.com](https://www.drugs.com/drug_interactions.html)")
    st.markdown("- [UpToDate](https://www.uptodate.com/drug-interactions)")
    st.markdown("- [CredibleMeds](https://crediblemeds.org)")
    st.markdown("- [CIMA AEMPS](https://cima.aemps.es/cima/)")

    st.success("✅ Informe generado con éxito. Puedes copiar las recomendaciones para tu informe clínico.")
